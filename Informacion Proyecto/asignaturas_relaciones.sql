...
CREATE TABLE IF NOT EXISTS `Iuteb_asignaturas`.`usuarios` (
  ...
  CONSTRAINT `user_id_malla`
    ...
    REFERENCES `Iuteb_asignaturas`.`malla_curricular` (`id`)
    ON DELETE RESTRICT
    ON UPDATE CASCADE)
ENGINE = InnoDB;
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `Iuteb_asignaturas`.`user_has_preferencias` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `id_usuario` INT NULL,
  `cant_posts` INT NULL DEFAULT 5,
  `color_ui` VARCHAR(45) NULL DEFAULT 'positive',
  `recibir_notificaciones` INT NULL DEFAULT 1,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_usuario_UNIQUE` (`id_usuario` ASC),
  CONSTRAINT `uhp_id_usuario`
    FOREIGN KEY (`id_usuario`)
    REFERENCES `Iuteb_asignaturas`.`usuarios` (`id_usuario`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
